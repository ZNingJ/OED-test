...
def ReadNABDataset(_file_name, _normalize=True):
    ...
    abnormal['label'] = 1
    abnormal=abnormal.sort_values(by='timestamp')
    list_windows = json_label.get(os.path.join(os.path.basename(os.path.dirname(_file_name)),os.path.basename(_file_name)).replace('\\','/'))
...
def CalculateROCAUCMetrics(_abnormal_label, _score):
    fpr, tpr, _ = roc_curve(_abnormal_label, _score,pos_label=1)
